...
/** 
 * @see 类名：NonIcvnPinLockQuery
 * @see 描述：查询ICVNs是否锁定，锁定true，没锁定false
 *
 * @see 创建日期：   2015年6月24日 下午2:41:36
 * @author yuyang
 * 
 * @see 修改记录：
 ...
 */
@Service
public class NonIcvnPinLockQuery implements IControlFieldStateQuery{
	...
	public boolean process(String cardNbr) throws ProcessException {
		logger.debug("查询ICVN是否锁定开始");
		Map<CntType, Boolean> map = mmCardService.MS3105(cardNbr, "");
		logger.debug("查询ICVN是否锁定结束");
		
		return map.get(CntType.icvn);
	}
