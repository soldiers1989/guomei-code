...
/** 
 * @see 类名：NonQPinLockQuery
 * @see 描述：查询查询密码是否锁定，锁定true，没有锁定false
 *
 * @see 创建日期：   2015年6月24日 下午2:42:04
 * @author yuyang
 * 
 * @see 修改记录：
 ...
 */
@Service
public class NonQPinLockQuery implements IControlFieldStateQuery{
	...
	public boolean process(String cardNbr) throws ProcessException {
		logger.debug("查询查询密码是否锁定开始,卡号后四位{}", CodeMarkUtils.subCreditCard(cardNbr));
		...
		logger.debug("查询查询密码是否锁定结束");
		
		return CcsCardO.getInqPinTries() >= productCredit.maxInqPinTry;
	}
