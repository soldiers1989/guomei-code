...
/**
 * @see 类名：NonCvv2PinLockQuery
 * @see 描述：查询CVV2是否锁定，锁定true，没锁定false
 *
 * @see 创建日期：   2015年6月24日 下午2:40:38
 * @author yuyang
 * 
 * @see 修改记录：
 ...
 */
@Service
public class NonCvv2PinLockQuery implements IControlFieldStateQuery{
	...
	public boolean process(String cardNbr) throws ProcessException {
		logger.debug("查询CVV2是否锁定开始");
		Map<CntType, Boolean> map = mmCardService.MS3105(cardNbr, "");
		logger.debug("查询CVV2是否锁定结束");
		
		return map.get(CntType.cvn2);
	}
