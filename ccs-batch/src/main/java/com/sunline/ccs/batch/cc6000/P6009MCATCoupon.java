 ...
 *
 */
public class P6009MCATCoupon implements ItemProcessor<S6000AcctInfo, S6000AcctInfo>{
	@PersistenceContext
	...
	public S6000AcctInfo process(S6000AcctInfo item) throws Exception {
		...
		logger.debug("优惠券（随借随还）处理：账户号["+item.getAccount().getAcctNbr()+"]，订单号：["+order.getOrderId()+"]");
		// 查找转入计划
		...
		//找不到转入计划，严重错误
		if(xfrInPlan==null){
			throw new Exception("找不到信用计划，账户号["+item.getAccount().getAcctNbr()+"]，订单号："+order.getOrderId()+"]");
		}
		// 交易主键生成
		...
		logger.debug("优惠券实际抵扣金额：["+creditAmt+"]");
		CcsPostingTmp post = generateTrans(txnSeq,item,creditAmt,xfrInPlan);
		txnPrepare.txnPrepare(post, null);
		// 优惠券抵扣交易优先入账
		item.getTxnPosts().add(0, post);
		return item;
	}
	...
	private CcsPostingTmp generateTrans(CcsTxnSeq txnSeq,S6000AcctInfo item,BigDecimal amt,CcsPlan xfrInPlan){
		SysTxnCdMapping sysTxnCdMapping = parameterFacility.loadParameter(SysTxnCd.C18, SysTxnCdMapping.class);
