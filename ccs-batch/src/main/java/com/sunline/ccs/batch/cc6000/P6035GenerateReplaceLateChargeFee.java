 ...
 */
@Component
public class P6035GenerateReplaceLateChargeFee implements ItemProcessor<S6000AcctInfo, S6000AcctInfo> {
	...
	public S6000AcctInfo process(S6000AcctInfo item) throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("收取代收滞纳金：Org["+item.getAccount().getOrg()
...
//		}
		...
		if (latePayAmt.compareTo(BigDecimal.ZERO) > 0) {
			// 生成一笔滞纳金收取交易，并入账
			CcsPostingTmp txnPost = generatorTransaction.generateLateChargeFee(item, latePayAmt, batchFacility.getBatchDate(), true);
