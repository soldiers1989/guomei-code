 ...
 * 流水对账公共方法
 */
public class MsxfMatchOrderCommon {
	 ...
	 * @param item
	 */
	public void autoRecovery(CcsOrder order, MsxfTranFlow item) {
		logger.info("对账普通异常,自动恢复");

		// 根据返回码获取订单状态
		MsPayfrontError msPayfrontError = getErrorEnum(item.msDdReturnCode, MsPayfrontError.class);
		...
		if (MsPayfrontError.S_0 == msPayfrontError) {
			...
			//
			if (StringUtils.equals(msPayfrontError.getDecision(),
					DecisionCode.P.name())) {
				// setErrorInfo(context,AuthTransStatus.P,OrderStatus.W,LoanRegStatus.C);
				updateOrder(order, item, OrderStatus.W, null);
			} else {
				// 交易失败
				if (LoanUsage.A == order.getLoanUsage()
						|| LoanUsage.L == order.getLoanUsage()) {
					// 代付需要额外设置
	...
	public void updateSettleLoanHst(CcsOrder order, Indicator settleInd) {
		...
		CcsSettleLoanHst ccsSettleLoanHst = new JPAQuery(em).from(q)
				.where(q.orderId.eq(order.getOrderId())).singleResult(q);
	...
	private void setErrorInfo(CcsOrder order, MsxfTranFlow item) {
		...
		// 对未匹配金额进行反向处理
		CcsAcctO accto = em.find(CcsAcctO.class,
				new CcsAcctOKey(order.getAcctNbr(), order.getAcctType()));
	...
	private CcsLoanReg getSinLoanReg(CcsOrder order) {
		...
		// 阳光保险 用保单号查 马上贷用借据号查
		BooleanExpression expression = q.loanRegStatus.eq(LoanRegStatus.C).and(
				q.refNbr.eq(order.getRefNbr()));
		...
		CcsLoanReg loanReg = new JPAQuery(em).from(q).where(expression)
				.singleResult(q);
		return loanReg;
	}
	 ...
	 */
	private void updateAuthMemoO(CcsAuthmemoO um, MsxfTranFlow item,
			CcsAcctO accto, AuthTransStatus authTransStatus) {
		MsPayfrontError msPayfrontError = this.getErrorEnum(
				item.msDdReturnCode, MsPayfrontError.class);
	...
	private void updateAcct(CcsOrder order, CcsLoanReg loanReg, CcsAcctO accto) {
		...
		CcsAcct acct = em.find(CcsAcct.class, new CcsAcctKey(
				order.getAcctNbr(), order.getAcctType()));
		logger.debug("分期计划参数[{}],[{}],[{}]", loanReg.getLoanCode(),
				loanReg.getLoanInitTerm(), loanReg.getLoanInitPrin());
		ProductCredit productCredit = unifiedParameterFacility.loadParameter(
				acct.getProductCd(), ProductCredit.class);
		LoanPlan loanPlan = unifiedParameterFacility.loadParameter(
				loanReg.getLoanCode(), LoanPlan.class);
		...
		if (null == acct.getLtdLoanAmt()
				|| acct.getLtdLoanAmt().compareTo(BigDecimal.ZERO) == 0) {
			acct = this.setNextStmtDate(acct, batchFacility.getBatchDate(),
					loanFeeDef, acctAttr, loanPlan);
			...
			if (null != accto) {
				accto.setCycleDay(String.format("%02d",
						Integer.valueOf(acct.getCycleDay())));
			}
		}
	...
	public CcsAcct setNextStmtDate(CcsAcct acct, Date bizDate,
		...
		if (!loanPlan.loanType.equals(LoanType.MCAT)) {
			fixDate = microCreditRescheduleUtils.skipEndOfMonth(acctAttr,
					bizDate);
		}
	...
	private void prepaymentProc(CcsOrder order) {
		...
		QCcsLoanReg q = QCcsLoanReg.ccsLoanReg;
		// 阳光保险 用保单号查 马上贷用借据号查
		BooleanExpression expression = q.loanRegStatus.eq(LoanRegStatus.A).and(
				q.loanAction.eq(LoanAction.O));
		expression = expression.and(q.contrNbr.eq(order.getContrNbr()));
		expression = expression.and(q.dueBillNo.eq(order.getDueBillNo()));
	...
	private String genAuthCode(Long logKv) {
		...
		String authCode = txnSeq.length() < 6 ? String.format("%06d",
				Integer.valueOf(txnSeq)) : StringUtils.right(txnSeq, 6);
		return authCode;
	}
	...
	private void updateAcctO(CcsAcctO accto) {
		...
		if (null == accto.getLastUpdateBizDate()
				|| accto.getLastUpdateBizDate().before(
	...
	private void subrogationProc(CcsOrder order, MsxfTranFlow item) {
		...
		claim.setCompensatoryAmt(claim.getCompensatoryAmt() == null ? item.txnAmt
				: claim.getCompensatoryAmt().add(item.txnAmt));
		claim.setLastCompensatoryDate(batchFacility.getBatchDate());
		// 更新订单
		updateOrder(order, item, OrderStatus.S, null);
		// 更新授权未匹配金额
		// CcsAcctO accto = em.find(CcsAcctO.class, new
		// CcsAcctOKey(order.getAcctNbr(), order.getAcctType()));
		// updateAcctoMemoAmt(accto, item.txnAmt, order.getTxnType(), true);
		// 入账流水
		// savePosting(order, item, null);
	}

	private CcsTxnPost savePosting(CcsOrder order, MsxfTranFlow item,
			CcsAuthmemoO um, CcsAcctO accto) {
		if (logger.isDebugEnabled()) {
		...
		txnPost.setTxnAmt(order.getTxnAmt());
		txnPost.setTxnCurrenctCode(order.getCurrency());
		txnPost.setSettAmt(order.getTxnAmt());
		...
		txnPost.setB011Trace(order.getRefNbr().substring(
				order.getRefNbr().length() - 9,order.getRefNbr().length() - 3));// 内部交易流水后6位随机数
	 ...
	 * @param authTransType
	 * @param amtFlag
	 *            compFlag 金额计算标识 true 加 false 减
	 */
	private void updateAcctoMemoAmt(CcsAcctO accto, BigDecimal txnAmt,
			AuthTransType authTransType, boolean amtFlag) {
		// 代付
		if (AuthTransType.AgentDebit == authTransType) {
			if (amtFlag) {
				accto.setMemoDb(accto.getMemoDb() != null ? accto.getMemoDb()
						.add(txnAmt) : txnAmt);
			} else {
				// 金额为空或负数时，修正为0
				accto.setMemoDb(accto.getMemoDb() != null
						&& accto.getMemoDb().compareTo(txnAmt) >= 0 ? accto
						.getMemoDb().subtract(txnAmt) : BigDecimal.ZERO);
			}
		} else if (AuthTransType.AgentCredit == authTransType) {
			// 代收
			if (amtFlag) {
				accto.setMemoCr(accto.getMemoCr() != null ? accto.getMemoCr()
						.add(txnAmt) : txnAmt);
			} else {
				// 金额为空时，修正为0
				accto.setMemoCr(accto.getMemoCr() != null
						&& accto.getMemoCr().compareTo(txnAmt) >= 0 ? accto
						.getMemoCr().subtract(txnAmt) : BigDecimal.ZERO);
			}
		}
	}
	...
	private void updateOrder(CcsOrder order, MsxfTranFlow item,OrderStatus orderStatus, Long logKv) {
		...
		order.setMemo(StringUtils.isBlank(order.getMemo()) ? "对账更新" : order
				.getMemo() + "-对账更新");
	...
	private CcsAuthmemoO saveCcsAuthmemoO(CcsOrder order, MsxfTranFlow item) {
		...
		um.setB011Trace(order.getRefNbr().substring(
				order.getRefNbr().length() - 9,order.getRefNbr().length() - 3));// 内部交易流水后6位随机数
	...
	private CcsSettleClaim matchSettleClaim(CcsOrder order) {
		...
		CcsSettleClaim settleClaim = query
				.from(qCcsSettleClaim)
				.where(qCcsSettleClaim.acctNbr
						.eq(order.getAcctNbr())
						.and(qCcsSettleClaim.guarantyId.eq(order
								.getGuarantyId()))
	...
	public <T extends Enum<T>> T getErrorEnum(String value, Class<T> clazz) {
		try {
			...
			throw new ProcessException(MsPayfrontError.E_90000.getCode(),
					MsPayfrontError.E_90000.getDesc());
		}
	}
}
