...
/**
 * 
 * @see 类名：AuthMicroCreditVoidedReverseHandler
 * @see 描述：[分期]撤销的冲正
 *
 * @see 创建日期：   2015年6月24日下午3:30:20
 * @author liruilin
 * 
 * @see 修改记录：
 ...
 */
@Service
public class AuthMicroCreditVoidedReverseHandler extends AuthVoidedReverseHandler {
	@Autowired
	...
	@Override
	protected void updateOriginalInfo(CcsAuthmemoO orig, CcsAuthmemoO origVoid, AuthContext context) throws AuthException {
		/**
		 * 分期处理：冲正交易改变LOAN_REG表中的分期注册状态为[A]
		 */
		CcsLoanReg loanReg = authCommonService.findLoanRegWithB7B11B32B33(orig,LoanRegStatus.V,context.getTxnInfo().getManualAuthFlag());
		if(loanReg != null){
