...
/**
 * 
 * @see 类名：CreditVoidHandler
 * @see 描述：贷记撤销 (存款)
 *
 * @see 创建日期：   2015年6月24日下午3:43:20
 * @author liruilin
 * 
 * @see 修改记录：
 ...
 */
@Service
public  class CreditVoidHandler implements Handler {
	...
	public RespInfo handle(YakMessage request, AuthContext context)
		...
		// 更新原交易记录
		orig.setAuthTxnStatus(AuthTransStatus.V);
		if (orig.getVoidCnt() == null) {
			orig.setVoidCnt(1);
		} else {
			orig.setVoidCnt(orig.getVoidCnt() + 1);
		}
