...
@Singleton
public class MerchantGroupPage extends Page {
	...
	public IsWidget createPage() {
        ...
        // 搜索列表
        form.setField(uMerchantGroup.MerGroupId(),uMerchantGroup.MerGroupName());
        form.setCol(2);
        ...
        // 编辑按钮
        if (Authorization.accredit(MerchantGroupDetailPage.class))
        {
            grid.addDblClickListener(new IDblClickRowEventListener()
            {
                ...
                {
                    String merGroupId = data.getString(uMerchantGroup.MerGroupId().getName());
                    Token token = Flat.get().getCurrentToken();
                    token.addParam(uMerchantGroup.MerGroupId().getName(), merGroupId);
                    token.directPage(MerchantGroupDetailPage.class);
                    Flat.get().goTo(token);
                }
            });
        }
        ...
        // 增加按钮
        if (Authorization.accredit(MerchantGroupAddPage.class))
        {
            grid.setHeader(ClientUtils.createAddItem(new IClickEventListener()
            {
                ...
                {
                    Token token = Flat.get().getCurrentToken();
                    token.directPage(MerchantGroupAddPage.class);
                    Flat.get().goTo(token);
                }
            }));
        }
        ...
        // 显示列表名
        grid.setColumns(uMerchantGroup.MerGroupId(),
        		uMerchantGroup.MerGroupName(),
        		uMerchantGroup.MerGroupDesc());
	...
	public void refresh() {
	    ...
	    form.getUi().clear();
	    grid.clearData();
            grid.loadDataFromUrl("rpc/merchantGroupServer/fetchMerchantGroupList");
	            ...
	            Dialog.confirm("是否确认要删除？", "提示", new ConfirmCallbackFunction()
	            {
	                ...
	                public void corfimCallback(boolean value)
	                {
	                    if (value)
	                    {
	                        ...
	                        {
	                            keys.add(listData.get(i).asMapData().getString(uMerchantGroup.MerGroupId().getName()));
	                        }
	                        RPC.ajax("rpc/merchantGroupServer/deleteMerchantGroup", new RpcCallback<Data>()
	                        {
	                            @Override
