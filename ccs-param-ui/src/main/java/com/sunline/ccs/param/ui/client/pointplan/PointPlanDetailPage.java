...
import com.sunline.ui.core.client.enums.FieldType;
...
@Singleton
public class PointPlanDetailPage extends SavePage {

	public static final String PRRAM_PAGE_ID = "";
    ...
    public void refresh() {
        ...
        key = token.getParam(uPointPlan.PlanNbr().getName());
		RPC.ajax("rpc/pointPlanServer/getPointPlan", new RpcCallback<Data>(){
			...
			mapdata.put("startDate" , format.format(new Date(Long.parseLong(result.asMapData().getString("startDate")))));
			mapdata.put("endDate" , format.format(new Date(Long.parseLong(result.asMapData().getString("endDate")))));

					detailForm.setFormData(result);
					txnFilterRuleLayout.updateView(result.asMapData().getData("filterRule"));
					pointAccumRuleLayout.updateView(result.asMapData().getData("accumRule"));
				}
		    }
	},key );
	}
	...
	public IsWidget createPage() {
	    ...
	    detailForm.setField(uPointPlan.PlanNbr().readonly(true),
	    		 uPointPlan.PlanType().required(true),
	    		startDate,
	    		 endDate,
	    		 uPointPlan.Description().asTextArea());
	    detailForm.setCol(2);
...
//	panel.add(tab);
	...
	TabItemSetting filterRuleTabSetting = new TabItemSetting(null, constants.pointTxnFilterRule());
	tab.addItem(filterRuleTabSetting, txnFilterRuleLayout.createCanvas());
	
	//积分规则layout

	TabItemSetting accumRuleTabSetting = new TabItemSetting(null, constants.pointAccumRule());
	...
	KylinButton submitBtn = ClientUtils.creConfirmButton(new IClickEventListener(){
		...
		public void onClick() {
			...
			//保存数据
			RPC.ajax("rpc/pointPlanServer/updatePointPlan", new RpcCallback<Data>(){
				@Override
				public void onSuccess(Data result) {
					notice(false);
