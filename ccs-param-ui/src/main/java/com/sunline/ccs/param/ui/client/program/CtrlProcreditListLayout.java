 ...
 */
@Singleton
public class CtrlProcreditListLayout extends SavePage {

    @Inject
    private ProgramConstants constants;

    private TextColumnHelper cardSelect;
    private KylinGrid grid;
	private static String CtrlProdcredit = "ctrlProdcredit";
	...
	grid = new KylinGrid();
		...
		cardSelect = new TextColumnHelper("ctrlProdcredit", constants.ctrlProdcredit(), 400);
		grid.setColumns(cardSelect.setColumnWidth(400).setColunmEditor(new Editor().type(EditorType.SELECT)).columnRender(enumMap));
		grid.getSetting().onBeforeEdit(new IBeforeEditListener() {
			...
			public boolean onBeforeEdit(GridEditParamEntity editParm) {
				...
				if(CtrlProdcredit.equals(editParm.getColumnName())) {
    ...
    public void updataView(ListData listData) {
    	...
    	RPC.ajax("rpc/ccsSelectOptionServer/getProdcreditList", new RpcCallback<Data>(){
			@Override
	...
	for(int i = 0; i < listData.toList().size(); i++)
	{
	    ...
	    dataMap.put("ctrlProdcredit", listData.getString(i));
	...
	for (int i=0; i < ld.size(); i++) {
	    if(ld.get(i).asMapData().getString("ctrlProdcredit")!=null
		    &&!ld.get(i).asMapData().getString("ctrlProdcredit").equals("")){
	    target.addString(ld.get(i).asMapData().getString("ctrlProdcredit"));
	    }
	}
	MapData md=new MapData();
	md.put("ctrlProdCreditList",target.asMapData());
	return md;
    }
